...
export const LOCALES = {
  ...
  fr: {
    ...
    ui: {
      ...
      statusNote: (d, repo) => `Statuts évalués le ${d} à partir de sources publiques — <strong>ils décrivent ce que les entreprises déclarent et déposent, pas des mesures de leurs logiciels</strong>, et l'absence de preuve n'est pas une preuve d'absence : un fournisseur pourrait scanner sans le déclarer. Les fiches détaillées des entreprises sont en anglais. Un statut vous semble faux ? <a href="${repo}/issues">Contestez-le</a> — les contestations sont publiées.`,
    },
    status: {
      ...
      global: { label: "Scanne mondialement — aucune preuve UE", verdict: "Scanne sous la loi américaine · aucune preuve UE",
        blurb: "Leurs documents révèlent un scan des contenus au titre du droit américain (signalements NCMEC, PhotoDNA). Aucune preuve qu'ils invoquent la dérogation européenne pour les communications privées — le scan « loi US » et le Chat Control sont deux régimes distincts. L'absence de preuve est un fait sur le dossier public, pas une preuve d'absence : un fournisseur pourrait scanner sans le dire." },
      unclear: { label: "Aucune déclaration claire", verdict: "Aucune déclaration trouvée",
        blurb: "Pas de chiffrement de bout en bout, et aucune déclaration publique claire, dans un sens ou dans l'autre, sur le scan des communications privées. Le silence mesure la transparence, pas le comportement." },
    ...
    home: {
      ...
      bignums: ["scannent sous Chat Control", "scannent sous la loi US · aucune preuve UE", "aucune déclaration trouvée", "affirme ne pas scanner", "ne peuvent pas — E2EE"],
  ...
  de: {
    ...
    ui: {
      ...
      statusNote: (d, repo) => `Status bewertet am ${d} auf Basis öffentlicher Quellen — <strong>sie beschreiben, was Unternehmen erklären und einreichen, keine Messungen ihrer Software</strong> — und fehlende Belege sind kein Beweis des Gegenteils: Ein Anbieter könnte scannen, ohne es offenzulegen. Die Detailseiten der Unternehmen sind auf Englisch. Ein Status erscheint Ihnen falsch? <a href="${repo}/issues">Widersprechen Sie</a> — Einwände werden veröffentlicht.`,
    },
    status: {
      ...
      global: { label: "Scannt weltweit — kein EU-Nachweis", verdict: "Scannt nach US-Recht · kein EU-Nachweis",
        blurb: "Ihre Dokumente belegen Inhalts-Scans nach US-Recht (NCMEC-Meldungen, PhotoDNA). Kein Nachweis, dass sie die EU-Ausnahmeregelung für private Kommunikation nutzen — US-Scans und Chatkontrolle sind getrennte Regime. Fehlende Belege sind eine Aussage über die öffentliche Aktenlage, kein Beweis des Gegenteils: Ein Anbieter könnte scannen, ohne es offenzulegen." },
      unclear: { label: "Keine klare Aussage", verdict: "Keine Aussage gefunden",
        blurb: "Nicht Ende-zu-Ende-verschlüsselt, und keine klare öffentliche Aussage — in keine Richtung — zum Scannen privater Kommunikation. Schweigen misst Offenlegung, nicht Verhalten." },
    ...
    home: {
      ...
      bignums: ["scannen unter der Chatkontrolle", "scannen nach US-Recht · keine EU-Belege", "keine Aussage gefunden", "erklärt, nicht zu scannen", "können nicht — E2EE"],
  ...
  es: {
    ...
    ui: {
      ...
      statusNote: (d, repo) => `Estados evaluados el ${d} a partir de fuentes públicas — <strong>describen lo que las empresas declaran y presentan, no mediciones de su software</strong>, y la ausencia de pruebas no es prueba de ausencia: un proveedor podría escanear sin declararlo. Las fichas detalladas de las empresas están en inglés. ¿Un estado le parece erróneo? <a href="${repo}/issues">Impúgnelo</a> — las impugnaciones se publican.`,
    },
    status: {
      ...
      global: { label: "Escanea globalmente — sin pruebas en la UE", verdict: "Escanea bajo ley de EE. UU. · sin pruebas UE",
        blurb: "Sus documentos revelan escaneo de contenidos bajo la ley estadounidense (informes al NCMEC, PhotoDNA). Ninguna prueba de que invoquen la excepción europea para comunicaciones privadas — el escaneo «ley de EE. UU.» y el Chat Control son regímenes distintos. La ausencia de pruebas es un hecho sobre el registro público, no una prueba de ausencia: un proveedor podría escanear sin declararlo." },
      unclear: { label: "Sin declaración clara", verdict: "Sin declaración encontrada",
        blurb: "Sin cifrado de extremo a extremo, y sin declaración pública clara, en ningún sentido, sobre el escaneo de comunicaciones privadas. El silencio mide la transparencia, no el comportamiento." },
    ...
    home: {
      ...
      bignums: ["escanean bajo Chat Control", "escanean bajo ley de EE. UU. · sin pruebas UE", "sin declaración encontrada", "afirma que no escanea", "no pueden — E2EE"],
  ...
  pl: {
    ...
    ui: {
      ...
      statusNote: (d, repo) => `Statusy ocenione ${d} na podstawie publicznych źródeł — <strong>opisują to, co firmy deklarują i składają, a nie pomiary ich oprogramowania</strong>, a brak dowodów nie jest dowodem nieistnienia: dostawca mógłby skanować i tego nie ujawniać. Szczegółowe strony firm są po angielsku. Status wygląda na błędny? <a href="${repo}/issues">Zakwestionuj go</a> — spory są jawne.`,
    },
    status: {
      ...
      global: { label: "Skanuje globalnie — brak dowodów z UE", verdict: "Skanuje na mocy prawa USA · brak dowodów z UE",
        blurb: "Ich dokumenty ujawniają skanowanie treści na mocy prawa amerykańskiego (raporty NCMEC, PhotoDNA). Nie znaleziono dowodów, by powoływali się na unijną derogację wobec prywatnej komunikacji — skanowanie „po amerykańsku” i kontrola czatów to dwa osobne reżimy. Brak dowodów to fakt o jawnym rejestrze, a nie dowód nieistnienia: dostawca mógłby skanować i tego nie ujawniać." },
      unclear: { label: "Brak jasnej deklaracji", verdict: "Brak znalezionej deklaracji",
        blurb: "Bez szyfrowania end-to-end i bez jasnej publicznej deklaracji — w żadną stronę — o skanowaniu prywatnej komunikacji. Cisza mierzy jawność, nie zachowanie." },
    ...
    home: {
      ...
      bignums: ["skanują w ramach kontroli czatów", "skanują na mocy prawa USA · brak dowodów z UE", "brak znalezionej deklaracji", "twierdzi, że nie skanuje", "nie mogą — E2EE"],
...
export function emitLocales(ctx) {
  ...
  <section class="hero">
    ...
    <div class="bignums">
      ${groups.map((g, i) => `<a href="#${g.key}"><b class="${["n-red", "n-amber", "n-gray", "n-greensoft", "n-green"][i]}">${g.companies.length}</b><span>${H.bignums[i]}</span></a>`).join("\n      ")}
    </div>
